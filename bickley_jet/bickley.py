...
def animate(iter, Q, dt, Xp, Yp, X, Y, phi_data, col_iter):
    ...
    ax.contour(X, Y, phi_data[:,:,iter+1],0,colors='k')
    ax.legend()
...
if TF_back_track and TF_four_closest and REACH:
    ...
    while k!=0:
        if l%size_backtrack==0:
            ...
            x_new = x_tmp_sub-dt*([u,v]+F*normal)*size_backtrack
            print('x_new',x_new)
...
# plt.title('single gyro w/ control F='+str(F)+',size of backtrack ='+str(size_backtrack))
