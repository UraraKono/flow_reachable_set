...
import time
# docking of two agents in tank

start_time = time.time()

# Load the velocity field using np.load
...
if os.path.exists('data/velocity_field_'+str(date)+'_res_'+str(resolution)+'concatenate.npy'):
    ...
    X, Y, grid_U, grid_V = np.load('data/velocity_field_'+str(date)+'_res_'+str(resolution)+'concatenate.npy')
# grid_V = -grid_V
# if os.path.exists('/home/urara/Documents/PEG_sim/TankData/data/velocity_field_'+str(date)+'_res_'+str(resolution)+'concatenate_xy_swap.npy'):
#     print('loading vector field')
#     X, Y, grid_U, grid_V = np.load('/home/urara/Documents/PEG_sim/TankData/data/velocity_field_'+str(date)+'_res_'+str(resolution)+'concatenate_xy_swap.npy')
# else:
#     print('creating vector field')
#     create_vec_field(date, resolution)
#     X, Y, grid_U, grid_V = np.load('/home/urara/Documents/PEG_sim/TankData/data/velocity_field_'+str(date)+'_res_'+str(resolution)+'concatenate_xy_swap.npy')
...
# This is already obtained from the npy file
...
# plt.colorbar(stream, cmap = plt.cm.jet,label='flow speed w[m/s]', shrink=0.6)
...
for i in range(int(T/dt)):
    ...
    if len(x_dock_index) > 0:
        ...
        print('two robots docking at t=',i*dt)
    ...
    for agent in range(n_agents):
        ...
        while k!=0:
            if l%size_backtrack==0:
                ...
                if n_zero_level<4:
                    print('not enough 0-level points')
                    break
                d_list = []
...
# print out the actual waypoints
waypoints_a = path*resolution+np.array([xmin,ymin])
waypoints_b = path_b*resolution+np.array([xmin,ymin])
waypoints_a = np.flipud(waypoints_a)
waypoints_b = np.flipud(waypoints_b)
waypoints = np.concatenate((waypoints_a,waypoints_b),axis=1)
print(waypoints,'waypoints')

ax.scatter(x_ini[0,0],x_ini[0,1],c='k',marker='o',label='start of agent 1')
ax.scatter(x_ini[1,0],x_ini[1,1],c='k',marker='^',label='start of agent 2')
ax.scatter(x_goal[0],x_goal[1],c='k',marker='x',label='docking point')
ax.legend(loc = 'lower right')
ax.set_title('Planned path for two agents')
plt.savefig('results_tank/invariant_dock_xini'+str(x_ini[0])+str(x_ini[1])+'.png',dpi=300,bbox_inches='tight')
# plt.show()

# Save patameters to a csv file
if TF_save_csv:
    with open('results_tank/Tank_dock_xini'+str(x_ini[0])+str(x_ini[1])+'.csv', mode='w') as f:
        print('save csv')
        f.write('F,'+str(F)+'\n')
        ...
        # Write the waypoints
        f.write('waypoints for agent 1 \n')
        f.write(str(waypoints_a)+'\n')
        f.write('waypoints for agent 2 \n')
        f.write(str(waypoints_b)+'\n')
