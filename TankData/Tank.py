...
date='12_08_2022_12_52_06'
resolution = 0.05
# If the interpolated vector field has already been created, load it
# Otherwise, create it using interpolate_vectorfield.py
if os.path.exists('data/velocity_field_'+str(date)+'_res_'+str(resolution)+'concatenate_xy_swap.npy'):
    print('loading vector field')
    X, Y, grid_U, grid_V = np.load('data/velocity_field_'+str(date)+'_res_'+str(resolution)+'concatenate_xy_swap.npy')
else:
...
plt.title('Single gyro w/ control F='+str(F)+'plot every '+str(plot_time_step*dt)+'[s]')
plt.savefig('./results_tank/singlegyro_F_'+str(F)+',plot_every_'+str(plot_time_step*dt)+'res'+str(resolution)+'.png')
